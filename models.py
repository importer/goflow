...
class Admin(object):
    ...
    def run_all_job(self, date_list):
        ...
        :return:
        """
        job_list = TaskDefine().get_valid_job()
        msg = ""
        for etl_day in date_list:
            print (len(job_list), etl_day)
            TaskInstance().create_job_task_instance(etl_day, job_list)
    ...
    def rerun_task(self, task_id, date_list, up_and_down=False, run_up=False, run_down=False, force=False):
        ...
        :param run_up:run upstream 前置依赖
        :param run_down: run downstream 后置依赖
        :return:
        """
        kafka_utils.setup_kafka()
        # run job define
        instance_list = []
        msg = ""
        run_type = ""
        for loop in ["looop"]:
            _ = loop
            job = TaskDefine().get_job_by_task_id(task_id_list=[task_id])
            if job and len(job) > 0:
                run_type="job"
                job_list = []
                if run_up:
                    job_list = TaskDependency().get_all_upstream(task_id)
                elif run_down:
                    job_list = TaskDependency().get_all_downstream(task_id)
                elif up_and_down:
                    up_job = TaskDependency().get_all_upstream(task_id)
                    down_job = TaskDependency().get_all_downstream(task_id)
                    if len(up_job) > 0:
                    ...
                    if force:
                        for etl_day in date_list:
                            tmp = TaskInstance().direct_run_single_job_task(etl_day, job)
                            if len(tmp) > 0:
                ...
                need_run_job_list = TaskDefine().get_job_by_task_id(task_id_list=job_list)
                if need_run_job_list and len(need_run_job_list) > 0:
                    for etl_day in date_list:
                        TaskInstance().create_job_task_instance(execute_date=etl_day, job_list=need_run_job_list)
                    msg = "generate {} TaskDefine task instance ".format(len(need_run_job_list) * len(date_list))
                break

            # run cron task
            cron = CronConf().get_cron_by_task_id(task_id=task_id)
            if cron:
                run_type = "cron"
                for etl_day in date_list:
                    tmp = TaskInstance().create_cron_task_instance(execute_date=etl_day, cron_list=[cron])
                    if len(tmp) > 0:
                        instance_list.extend(tmp)
                break
    ...
    def migrate_dependency(self,  job_id=None, delete=False):
        ...
        :return:
        """
        TaskDependency().migrateDep(job_id=job_id, delete=delete)

    def migrate_lastid(self, etl_day):
        migrate_last_id(etl_day)

    def query_dep(self, task_id, etl_day):
        met, msg = TaskDependency(). \
    ...
    def kill_task(self, task_id, date_list):
        ...
        :return:
        """
        return TaskInstance().kill_instance(task_id, date_list)